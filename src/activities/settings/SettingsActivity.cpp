...
void SettingsActivity::rebuildSettingsLists() {
  ...
  // The on-screen keyboard (and with it the layout picker) only exists on
  // touch devices; button-only devices type through T4 instead (row below).
  if (BoardConfig::hasTouch()) {
  ...
  if (setting.type == SettingType::TOGGLE && setting.valuePtr != nullptr) {
    ...
    switch (setting.action) {
      ...
      case SettingAction::KeyboardLayouts:
        ...
        break;
      case SettingAction::AdditionalKeyboardLayout:
        startActivityForResult(std::make_unique<T4LayoutSelectActivity>(renderer, mappedInput), resultHandler);
        break;
