...
void KOReaderSettingsActivity::handleSelection() {
  if (selectedIndex == 0) {
    // Username
    startActivityForResult(textentry::makeEntryActivity(renderer, mappedInput, tr(STR_KOREADER_USERNAME),
    ...
    // Password
    startActivityForResult(textentry::makeEntryActivity(renderer, mappedInput, tr(STR_KOREADER_PASSWORD),
    ...
    startActivityForResult(
        textentry::makeEntryActivity(renderer, mappedInput, tr(STR_SYNC_SERVER_URL), prefillUrl, 128, InputType::Url),
        [this](const ActivityResult& result) {
          if (!result.isCancelled) {
            const auto& kb = std::get<KeyboardResult>(result.data);
